...
app.get('/room/:room', (req, res) => {
    ...
    res.render('room', {roomName: roomName, users: rooms[roomName].users, host: rooms[roomName].host}) // render room.ejs
})
...
app.post('/game', (req,res) => {
    ...
    problems = []
    for (var i = 0; i < 240; i++){
        var num1 = Math.floor(Math.random()*120)+1
        var num2 = Math.floor(Math.random()*120)+1
        var ans = num1+num2
        problems.push([num1, num2, ans])
    }
...
app.get('/game/:room', (req, res) => {
    roomName = req.params.room
    if (rooms[roomName] == null) { // if room does not exist
        return res.redirect('/')
    } else if (rooms[roomName].active == false) { // if room exists but game is not active
        return res.redirect('/room/'+roomName)
    }
    res.render('game', {roomName: roomName, problems: JSON.stringify(rooms[roomName].problems)})
})
...
io.on('connection', socket => {
    ...
    socket.on('new-user', room => {
        ...
        socket.broadcast.to(room).emit('user-connected', users[userRef]["username"])
        ...
        } else if (rooms[room].host == users[userRef]["UID"]){ // host exists and is the same person (e.g. they refreshed)
            socket.emit('add-host-to-user-list', users[userRef]["username"])
        } else {
            socket.emit('add-to-user-list', users[userRef]["username"]) // add self to list
            socket.broadcast.to(room).emit('add-to-user-list', users[userRef]["username"]) // tell everyone else to add you to list
        }
    })

    socket.on('send-chat-message', (room, message) => {
        socket.broadcast.to(room).emit('chat-message', {message: message, 
            name: rooms[room].users[userData["UID"]]["username"]}) // sends to every other user except for original user
